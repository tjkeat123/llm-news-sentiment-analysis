...
def analyze_news(ticker, num_articles=10, verbose=False):
    ...
    if verbose:
        print(f"Analyzing articles with Gemini AI...")
    gemini = Gemini()
    response_json = gemini.decide_news_importance(news_json)
    
    # Step 3: Merge news with importance decisions
    merged_json = merge_important_info_from_json(news_json, response_json)
    
    # Step 4: Get overall analysis score
    if verbose:
        print(f"Getting overall analysis score...")
    score_json = gemini.get_analysis_score(merged_json)
