...
# Note: xAI's API is OpenAI-compatible, so we use the OpenAI SDK with xAI's base URL.
# The xai-sdk package uses a different gRPC-based API which doesn't support structured outputs the same way.
    ...
    def decide_news_importance(self, news_articles_json):
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a helpful research assistant that decides if a news article is worth reading based on the title and summary."
                },
                {
                    "role": "user",
                    "content": prompt
            ...
            response_format={
                "type": "json_schema",
                "json_schema": {
                    "name": "NewsImportanceResponse",
                    "schema": NewsImportanceResponse.model_json_schema()
                }
            }
        )
    ...
    def get_analysis_score(self, news_articles_json):
        ...
        response = self.client.chat.completions.create(
            ...
            response_format={
                "type": "json_schema",
                "json_schema": {
                    "name": "NewsAnalysisScoreResponse",
                    "schema": NewsAnalysisScoreResponse.model_json_schema()
                }
            }
        )
